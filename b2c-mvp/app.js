...
let attachments=[],historyItems=[];
const SOCRATES_PROFILE='./assets/socrates-friend.webp';
let STUDENT={name:'민준',avatar:'👦',grade:'중2'};
try{STUDENT={...STUDENT,...JSON.parse(localStorage.getItem('ttai-profile')||'{}')}}catch(e){}
...
function applyProfile(){
  ...
  if(summary){const av=summary.querySelector('.profile-avatar'),name=summary.querySelector('strong'),grade=summary.querySelector('small');if(av)av.textContent=STUDENT.avatar||'👦';if(name)name.textContent=STUDENT.name||'학생';if(grade)grade.textContent=STUDENT.grade||'중2'}
...
function addMessage(role,text,items=[]){
  ...
  const name=document.createElement('div');name.className='message-name';name.textContent=role==='student'?(STUDENT.name||'학생'):'소크라테스';stack.appendChild(name);
  ...
  if(role==='student')row.append(stack,profile);else row.append(profile,stack);
  dialogueScroll.appendChild(row);addHistory(role==='student'?'내 생각':'소크라테스',text||'파일 첨부');requestAnimationFrame(()=>dialogueScroll.scrollTo({top:dialogueScroll.scrollHeight,behavior:'smooth'}));
}
...
document.querySelectorAll('[data-attach]').forEach(b=>b.onclick=()=>{const type=b.dataset.attach;attachMenu.hidden=true;attachButton.setAttribute('aria-expanded','false');if(type==='image')imageInput.click();else if(type==='file')fileInput.click();else alert('MVP에서는 카메라 연결 UI만 준비되었습니다. 실제 카메라 촬영은 다음 연결 단계에서 활성화합니다.')});
...
function sendCurrent(){const text=thoughtInput.value.trim();if(!text&&!attachments.length){thoughtInput.focus();return}addMessage('student',text,[...attachments]);thoughtInput.value='';attachments=[];renderPreview();autoSize();setTimeout(()=>addMessage('coach','좋아. 그 생각에서 가장 중요한 이유 하나만 골라서 말해볼래?'),450)}
