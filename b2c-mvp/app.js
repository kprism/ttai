...
function setDrawer(open){
  pathDrawer.hidden=!open;
  backdrop.hidden=!open;
  pathButton.setAttribute('aria-expanded',String(open));
}
...
companionToggle.addEventListener('click',()=>{
  const next=companionToggle.getAttribute('aria-pressed')!=='true';
  companionToggle.setAttribute('aria-pressed',String(next));
  companionPanel.hidden=!next;
  avatar?.classList.toggle('state-listen',next);
});

voiceButton.addEventListener('click',()=>{
  const active=voiceButton.dataset.active==='1';
  voiceButton.dataset.active=active?'0':'1';
  voiceButton.textContent=active?'🎤 말하기':'■ 듣고 있어요';
  avatar?.classList.toggle('state-listen',!active);
});
...
function renderPastePreview(){
  ...
  pastedImages.forEach((item,index)=>{
    ...
    meta.innerHTML=`<strong>붙여넣은 이미지</strong><span>${Math.round(item.file.size/1024)} KB</span>`;
    const remove=document.createElement('button');
    remove.type='button';
    remove.className='paste-remove';
    remove.setAttribute('aria-label','붙여넣은 이미지 삭제');
    remove.textContent='×';
...
thoughtInput.addEventListener('paste',event=>{
  ...
  if(text){
    const start=thoughtInput.selectionStart;
    const end=thoughtInput.selectionEnd;
    thoughtInput.value=thoughtInput.value.slice(0,start)+text+thoughtInput.value.slice(end);
  }
  renderPastePreview();
});

function makeHistoryChip(label,summary,index){
  const chip=document.createElement('button');
  chip.type='button';
  chip.className='history-chip';
  chip.dataset.turn=String(index);
  chip.innerHTML=`<small>${label}</small><span>${summary}</span>`;
  historyRail.appendChild(chip);
  chip.addEventListener('click',()=>scrollToTurn(index));
}

function bindHistoryChips(){
  document.querySelectorAll('.history-chip').forEach(chip=>{
    chip.addEventListener('click',()=>scrollToTurn(Number(chip.dataset.turn)));
  });
}
function scrollToTurn(index){
  const turns=[...document.querySelectorAll('.turn')];
  turns[index]?.scrollIntoView({behavior:'smooth',block:'center'});
  document.querySelectorAll('.history-chip').forEach((chip,i)=>chip.classList.toggle('active',i===index));
}
bindHistoryChips();
...
sendButton.addEventListener('click',()=>{
  ...
  turn.className='turn student-turn new-turn';
  turn.dataset.summary=text||'이미지로 생각을 공유했어.';
  const label=document.createElement('p');
  label.className='turn-label';
  label.textContent='내 생각';
  ...
  if(pastedImages.length){
    ...
    pastedImages.forEach(item=>{
      const img=document.createElement('img');
      img.src=item.url;
      img.alt='대화에 첨부한 캡처 이미지';
      gallery.appendChild(img);
    });
    turn.appendChild(gallery);
  }
  if(text){
    const answer=document.createElement('div');
    answer.className='student-answer';
